...
import { createRequest } from '../api/marketplace';
import { CATEGORIES } from '../constants';

export default function CreateRequest() {
    const navigate = useNavigate();
    const [form, setForm] = useState({
        title: '', description: '', category: 'OTHER', maxPrice: '',
    });
    ...
    return (
        <div className="max-w-xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4 shadow-sm">
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Бюджет (SC)</label>
                    <input type="number" min="0" className="input w-full" value={form.maxPrice}
                           onChange={e => setForm({ ...form, maxPrice: e.target.value })} />
                </div>
                <button type="submit" disabled={loading} className="btn-primary w-full py-3">
                    {loading ? 'Создание...' : 'Опубликовать заявку'}
                </button>
            </form>
        </div>
    );
